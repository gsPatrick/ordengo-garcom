...
export function CallingNotificationDialog({ table, isOpen, onConfirm, onCancel, isResolving }) {
  ...
  const getPaymentDetails = (method) => {
    switch(method) {
      case 'credit': return { label: 'Cartão de Crédito', icon: <CreditCard className="h-6 w-6" /> };
      case 'debit': return { label: 'Cartão de Débito', icon: <CreditCard className="h-6 w-6" /> };
      case 'pix': return { label: 'PIX', icon: <QrCode className="h-6 w-6" /> };
      case 'cash': return { label: 'Dinheiro', icon: <Banknote className="h-6 w-6" /> };
      default: return null;
    }
  };
  ...
  const callInfo = {
    title: isPaymentCall ? "Pedido de Conta" : "Chamado do Garçom",
    description: isPaymentCall ? "O cliente solicitou o fechamento da conta." : "O cliente está chamando na mesa.",
    icon: isPaymentCall ? 
        ...
        <AlertDialogHeader className="flex flex-col items-center text-center space-y-4 pt-6">
          ...
          {paymentInfo && (
            <div className="w-full bg-gray-100 border-2 border-gray-200 rounded-xl p-4 flex flex-col items-center justify-center gap-2 mt-2 animate-in zoom-in duration-300">
              <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Forma de Pagamento</span>
