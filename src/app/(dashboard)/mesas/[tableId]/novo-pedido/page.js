      ...
      <DialogContent className="sm:max-w-sm">
        ...
        <DialogFooter>
          <Button onClick={() => onConfirm(product, qty, obs)} className="w-full bg-[#df0024] hover:bg-red-700">
            Adicionar {Number(product.price * qty).toLocaleString('es-ES', { style: 'currency', currency: 'EUR' })}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      if (error.response?.data?.error?.code === '22P02') {
        alert("Error crítico: ID del restaurante inválido. Por favor, vuelva a iniciar sesión.");
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
          {categories.map(cat => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-colors ${activeCategory === cat.id
      ...
      {cart.length > 0 && (
        <div className="absolute bottom-0 left-0 right-0 bg-white border-t p-4 shadow-[0_-4px_20px_rgba(0,0,0,0.1)] animate-in slide-in-from-bottom-4">
          <div className="flex justify-between items-center mb-4">
            <span className="text-sm font-medium text-gray-500">{cart.length} items agregados</span>
          ...
          >
            {sending ? <Loader2 className="animate-spin" /> : (
              <>Enviar a Cocina <CheckCircle className="ml-2 h-5 w-5" /></>
            )}
          </Button>
        </div>
      )}
